 ...
 * - It will remove tasks
 * - It will return a task
 * 
 * - It will listen for task triggers.
 * - It will handle task timeouts
 * - It will listen for completed tasks
 *   - A recurrent task should be scheduled again
 *   - A task over maxRuns should be removed
 * 
 * Scheduler should emit a task.complete event
 * Scheduler should emit a task.created event
 * Scheduler should emit a task.removed event
...
class Scheduler extends EventEmitter {
    ...
    /**
     * Add a new task from an object defining
     * a task.
     * 
     * @param {Object} task Task definition
     */
    add(task) {
        return this.makeTask(task, true);
    }

    async remove(task) {
        try {
            task = await this.makeTask(task, false);
        } catch (error) {
            return error;
        }
    ...
    get(id) {
        return this.makeTaskFromId(id);
    }

    async makeTask(task, persist = true) {
        if (this.parser.isSerialized(task)) {
            task = {
                serializedTask: this.parser.deserialize(task)
            };
        } else if (typeof task === 'string') {
            /**
             * A string might be a task id
             * scheduler:tasks:9febf4b1-b30a-461a-91a6-248516bf6a80
             * Or just a string:
             * 9febf4b1-b30a-461a-91a6-248516bf6a80
             */
            if (!this._isTaskId(task)) {
                task = this._makeTaskIdFromString(task);
            }

            task = await this.makeTaskFromId(task);

        } else if (task instanceof Task) {} else if (typeof task === 'object') {
            task = new Task(task);
        } else {
            /**
             * We don't support arrays, booleans and numbers
             * ! We might want to raise an error here?
             */
            return Promise.reject(new Error('Unable to create task'));
        }

        if (!task.client) task.client = this.client;

        try {
            persist && await task.createIfNew();
        } catch (err) {
            return reject(err);
        }
    ...
    makeTaskFromId(taskId) {
        return new Promise((resolve, reject) => {
            ...
            this.client.get(taskId, async(err, serializedTask) => {
                ...
                if (!serializedTask) return resolve(false);
    ...
    subscribeToKeyEvents() {
        ...
        /**
         * This handles adding new tasks to Redis set.
         * 
         * TODO: We don't want a loop cycle of creating a
         * new task and handling this event. It should be
         * idempotent and not waste resources.
         * `__key*__:waterline:service:id:*`
         */
        this.pubsub.on('pmessage', (channel, message) => {
            console.log('+ pmessage', channel, message);
            this.onNewTaskEvent(channel, message);
        });

        /**
         * This handles a task TTL which we use to schedule
         * next run.
         * 
         * TODO: What happens if the task is not collected?
         * It would be the case if none of our workers are 
         * active or if the scheduler is down to catch that 
         * message. We could probably handle that edge case
         * by building the logic in Redis itself with Lua 
         * scripts (or eventually a native module)
         * Another approach would be to namespace events to 
         * a scheduler's ID. When we register a task trigger
         * we register two keys with TTL:
         * scheduler:<SCHEDULER_ID>:tasks:<TASK_ID>:ttl
         * scheduler:tasks:<TASK_ID>:ttl -> ttl2 = ttl + timeout 
         * 
         * `__keyevent@0__:expired`
         */
        this.pubsub.on('message', (channel, message) => {
            console.log('- Expired notification:', channel, message);
            this.onTaskExpirationTrigger(channel, message);
        });
    }
     ...
     */
    onTaskExpirationTrigger(channel, message) {
        const taskId = this._getTaskIdFromExpireKey(message);
        ...
        console.log('onTaskExpirationTrigger', taskId);

        /**
         * We retrieve the serialized task and hydrate it
         */
        this.makeTaskFromId(taskId).then(task => {
            if (!task) {
                return this.logger.info('We could not find task %s', taskId);
            }
            ...
            this.logger.error('Error getting task: %s', taskId, err);
        });
    }
     ...
     */
    onNewTaskEvent(channel, message) {
        /**
    ...
    //trigger a task. 
    //TODO: Remove
    createTaskFromSource(id) {
        console.log('creating task from source....');
        return new Promise((resolve, reject) => {
            this.client.get(id, async(err, source) => {
                if (err) return reject(err);
                if (!source) return resolve({ found: false, id: id });
    ...
    _isTaskId(str) {
        return !!str.match(/^(scheduler:tasks:[^:]*)$/);
    }

    _makeTaskIdFromString(id) {
        return `scheduler:tasks:${id}`;
    }

    _getTaskIdFromExpireKey(message) {
    ...
    _makeTaskIdFromRecord(message) {
        ...
        const id = match[1];
        return this._makeTaskIdFromString(id);
    }
