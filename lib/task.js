...
class Task {
    ...
    shouldRun(status, options = {}) {
        ...
        handlers.error = (options = {}) => {
            ...
            //if options.error.fatal state.status = fail
            if (this.errorCount < this.maxTries) {
                // state.status = 'enqueue';
                state.status = STATE.delayed;
                /**
                 * exponential back off
                 */
                state.ttl = this.backOff.compute(this.errorCount, options);

            } else state.status = STATE.failed;

            return state;
        };

        handlers.join = (options = {}) => {

        };

        handlers.commit = (options = {}) => {
            let state = { status: STATE.enqueued };
    ...
    toJSON() {
        ...
        //TODO: check scheduleOptions if its date then turn to ISO861
